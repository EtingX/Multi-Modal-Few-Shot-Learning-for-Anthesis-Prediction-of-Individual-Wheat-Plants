...
model_save_dir = 'TPA 2 swin_b_tf'
...
save_dir = os.path.join(home_dir, model_save_folder, model_save_dir, str(epoch), 'TRF 2023 result')
stand_save_model_ft = os.path.join(home_dir, model_save_folder, model_save_dir,str(epoch), 'TRF 2023 output 1')
stand_save_total = os.path.join(home_dir, model_save_folder, model_save_dir, str(epoch), 'TRF 2023 stand vector save')

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

if not os.path.exists(stand_save_model_ft) or not os.path.isdir(stand_save_model_ft):

    vector_generation(img_folder_dir, weather_folder_dir, model_dir, model_save_dir, stand_save_model_ft) # model_ft
# #
